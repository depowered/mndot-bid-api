...
from mndot_bid_api import enums, exceptions, schema
...
from mndot_bid_api.etl.load.bidders import load_bidders_quiet
from mndot_bid_api.etl.load.bids import load_bids_quiet
from mndot_bid_api.etl.load.contract import load_contract_quiet
from mndot_bid_api.etl.scrape import get_abstract_csv_path
from mndot_bid_api.etl.transform import (transform_bidders, transform_bids,
                                         transform_contract)
...
async def async_abstract_etl_pipeline(
    ...
    try:
        ...
        update_data = schema.AbstractETLUpdateData(
            stage=enums.AbstractETLStage.FETCH, successful=False, error_msg=err.args[0]
        )
        abstract_etl_interface.update(etl_id, update_data.dict(exclude_none=True))
        return
    ...
    try:
        ...
        update_data = schema.AbstractETLUpdateData(
            stage=enums.AbstractETLStage.EXTRACT,
            successful=False,
    ...
    try:
        ...
        update_data = schema.AbstractETLUpdateData(
            stage=enums.AbstractETLStage.TRANSFORM,
            successful=False,
            error_msg=err.args[0],
        )
        abstract_etl_interface.update(etl_id, update_data.dict(exclude_none=True))
    ...
    # Load Results
    try:
        _ = load_contract_quiet(transformed_contract, contract_interface)
        _ = load_bids_quiet(
            transformed_bids, bid_interface, item_interface, invalid_bid_interface
        )
        _ = load_bidders_quiet(transformed_bidders, bidder_interface)
    except exceptions.HTTPException as err:
        update_data = schema.AbstractETLUpdateData(
            stage=enums.AbstractETLStage.LOAD, successful=False, error_msg=err.args[0]
        )
        abstract_etl_interface.update(etl_id, update_data.dict(exclude_none=True))
        return

    # Update the AbstractETL record to successfully completed
    update_data = schema.AbstractETLUpdateData(
        stage=enums.AbstractETLStage.COMPLETE, successful=True
    )
    abstract_etl_interface.update(etl_id, update_data.dict(exclude_none=True))
